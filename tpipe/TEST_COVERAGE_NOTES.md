...
| 005.gate-first | first, lastRc | `TPipe.first -- yes` answers in at most **750 ms** (PLAN gate 250 ms; measured 41/42 ms), with `RESULT='y'` and `lastRc` 141 or 143 | contract | §2.6, **F4** — run in a child under `timeout 20`, so a regression that hangs `wait` fails the case instead of the suite |
