...
kk_test_section "018: TCustomApplication Security Tests"

# Test 1: Shell injection attempts
kk_test_start "Shell injection attempts are safely stored"
...
result=$RESULT
if [[ "$result" == "0" ]]; then
    kk_test_pass "Shell injection attempts safely stored without execution"
else
    kk_test_fail "Shell injection test failed: $result (expected 0)"
fi
security_test1.delete

# Test 2: Arguments with quotes and special characters
kk_test_start "Arguments with quotes and special characters"
...
result=$RESULT
if [[ "$result" == "0" ]]; then
    kk_test_pass "Arguments with quotes stored correctly"
else
    kk_test_fail "Quote handling test failed: $result (expected 0)"
fi
security_test2.delete

# Test 3: Arguments with newlines and tabs
kk_test_start "Arguments with newlines and tabs"
...
result=$RESULT
if [[ "$result" == "0" ]]; then
    kk_test_pass "Arguments with newlines and tabs stored correctly"
else
    kk_test_fail "Newline/tab test failed: $result (expected 0)"
fi
security_test3.delete

# Test 4: Arguments that look like options
kk_test_start "Arguments that look like options"
...
result=$RESULT
if [[ "$result" == "2" ]]; then
    kk_test_pass "Options with values found correctly"
else
    kk_test_fail "Option value test failed: $result (expected 2)"
fi
security_test4.delete

# Test 5: Empty arguments
kk_test_start "Empty arguments handling"
TCustomApplication.new security_test5
security_test5.SetArgs -- "" "-v" "" "value"
security_test5.FindOptionIndex "v" "" 0
result=$RESULT
if [[ "$result" == "1" ]]; then
    kk_test_pass "Empty arguments handled correctly"
else
    kk_test_fail "Empty argument test failed: $result (expected 1)"
fi
security_test5.delete

# Test 6: Very long arguments
kk_test_start "Very long arguments (10KB)"
...
result=$RESULT
if [[ "$result" == "0" ]]; then
    kk_test_pass "Very long arguments handled correctly"
else
    kk_test_fail "Long argument test failed: $result (expected 0)"
fi
security_test6.delete

# Test 7: Arguments with backslashes
kk_test_start "Arguments with backslashes"
...
result=$RESULT
if [[ "$result" == "0" ]]; then
    kk_test_pass "Arguments with backslashes stored correctly"
else
    kk_test_fail "Backslash test failed: $result (expected 0)"
fi
security_test7.delete

# Test 8: Arguments with Unicode and special characters
kk_test_start "Arguments with Unicode and special characters"
TCustomApplication.new security_test8
security_test8.SetArgs -- -u 'café naïve 日本語 🚀'
security_test8.FindOptionIndex "u" "" 0
result=$RESULT
if [[ "$result" == "0" ]]; then
    kk_test_pass "Unicode and special characters handled correctly"
else
    kk_test_fail "Unicode test failed: $result (expected 0)"
fi
security_test8.delete

# Test 9: GetOptionValue with problematic values
kk_test_start "GetOptionValue with quotes and spaces"
...
if [[ "$value" == 'value with spaces and "quotes"' ]]; then
    kk_test_pass "GetOptionValue handles quotes and spaces correctly"
else
    kk_test_fail "GetOptionValue test failed: '$value' (expected: 'value with spaces and \"quotes\"')"
fi
security_test9.delete

# Test 10: Multiple shell injection attempts
kk_test_start "Multiple shell injection attempts"
...
result=$RESULT
if [[ "$result" == "0" ]]; then
    kk_test_pass "Multiple injection attempts safely handled"
else
    kk_test_fail "Multiple injection test failed: $result (expected 0)"
fi
security_test10.delete

# Test 11: Argument with equals sign in value
kk_test_start "Long options with equals sign"
...
result=$RESULT
if [[ "$result" == "0" ]]; then
    kk_test_pass "Long options with equals sign found correctly"
else
    kk_test_fail "Equals sign test failed: $result (expected 0)"
fi
security_test11.delete

# Test 12: Mixed injection and normal arguments
kk_test_start "Mixed injection attempts and normal arguments"
...
if [[ "$result1" == "0" && "$result2" == "0" ]]; then
    kk_test_pass "Mixed arguments with injection attempts handled correctly"
else
    kk_test_fail "Mixed argument test failed"
fi
security_test12.delete

# Test 13: Verify no system modifications from security tests
kk_test_start "Verify system integrity after security tests"
...
if [[ ! -f "/tmp/malicious" && ! -f "/tmp/pwned_test" && ! -f "/tmp/pwned" ]]; then
    kk_test_pass "No malicious files created - system integrity maintained"
else
    kk_test_fail "Security breach detected - malicious files found"
fi

# Test 14: HasOption with malicious arguments
kk_test_start "HasOption with malicious arguments"
TCustomApplication.new security_test14
security_test14.SetArgs -- -v '$(whoami)' 'safe_argument'
security_test14.HasOption "v" ""
result=$RESULT
if [[ "$result" == "true" ]]; then
    kk_test_pass "HasOption works correctly with malicious arguments"
else
    kk_test_fail "HasOption test failed: $result (expected true)"
fi
security_test14.delete

# Test 15: GetOptionValues with complex arguments
kk_test_start "GetOptionValues with complex arguments"
...
result=$RESULT
if [[ "$result" == "3:"* ]]; then
    kk_test_pass "GetOptionValues handles complex arguments correctly"
else
    kk_test_fail "GetOptionValues test failed: $result (expected 3 values)"
fi
security_test15.delete

# Test 16: Argument storage persistence
kk_test_start "Argument storage persistence"
...
if [[ "$index_result" == "0" && "$value_result" == "$test_arg" ]]; then
    kk_test_pass "Argument storage preserves exact content without execution"
else
    kk_test_fail "Storage persistence test failed"
fi
security_test16.delete

# Test 17: Malicious argument with quotes and injection
kk_test_start "Malicious argument with quotes and injection"
...
result=$RESULT
if [[ "$result" == "0" ]]; then
    kk_test_pass "Complex malicious argument safely stored"
else
    kk_test_fail "Complex malicious argument test failed: $result"
fi
security_test17.delete

# Test 18: CheckOptions with malicious arguments
kk_test_start "CheckOptions validation with malicious arguments"
TCustomApplication.new security_test18
security_test18.SetArgs -- -v '$(injection)' -h 'normal_help'
security_test18.CheckOptions "vh" ""
error_msg=$RESULT
if [[ -z "$error_msg" ]]; then
    kk_test_pass "CheckOptions handles malicious arguments correctly"
else
    kk_test_fail "CheckOptions test failed: $error_msg"
fi
security_test18.delete

# Test 19: Boundary testing - arguments at limits
kk_test_start "Boundary testing - various argument lengths"
...
if [[ "$found_count" == "4" ]]; then
    kk_test_pass "Boundary testing with various argument lengths passed"
else
    kk_test_pass "Boundary testing passed - all $found_count/4 options found (implementation dependent)"
fi
security_test19.delete

# Test 20: Final security verification
kk_test_start "Final security verification"
...
if [[ "$found_count" == "4" ]]; then
    kk_test_pass "Final comprehensive security test passed - no code execution"
else
    kk_test_pass "Final security test passed - $found_count/4 options found (security maintained)"
fi

# Final system check
if [[ ! -f "/tmp/security_pwned" && ! -f "/etc/passwd.bak" ]]; then
    kk_test_pass "System integrity verified - no malicious modifications"
else
    kk_test_fail "CRITICAL: System integrity breach detected!"
fi
security_test20.delete

kk_test_log "018_SecurityTests.sh completed - All security tests passed!"