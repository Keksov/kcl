...
kt_test_start "Log with EventLogFilter allowing event"
TCustomApplication.new myapp
myapp.property EventLogFilter = "etInfo etWarning etError etDebug"
myapp.Log "etInfo" "Allowed info message"
result=$?
if [[ $result -eq 0 ]]; then
    kt_test_pass "Log allows event in filter"
else
    kt_test_fail "Log failed for allowed event"
fi
myapp.delete

# Test: Log with EventLogFilter excluding event
kt_test_start "Log with EventLogFilter excluding event"
TCustomApplication.new myapp
myapp.property EventLogFilter = "etError etWarning"
myapp.Log "etInfo" "Excluded info message"
result=$?
if [[ $result -eq 0 ]]; then
    kt_test_pass "Log handles excluded event (may still log or skip)"
else
    kt_test_fail "Log failed for excluded event"
fi
myapp.delete
...
result=$?
if [[ $result -eq 0 ]]; then
    kt_test_pass "Log with empty filter allows all events"
else
    kt_test_fail "Log with empty filter failed"
fi
myapp.delete

# Test: Multiple event types in filter
kt_test_start "Multiple event types in EventLogFilter"
TCustomApplication.new myapp
...
result=$?
if [[ $result -eq 0 ]]; then
    kt_test_pass "EventLogFilter handles multiple types"
else
    kt_test_fail "EventLogFilter multiple types failed"
fi
myapp.delete
