...
|---|---|---|---|---|
| `tsed` (built) | GNU sed 4.9 | `expr` (the first `-e`) **plus** `addExpr`/`clearExprs` and `scriptFile` (`-f`); `extended` (`-E`), `quiet` (`-n`), `separate` (`-s`), `nullData` (`-z` → derived `-0`), `binary` (`-b`), **`sandbox` (default 1)**, `inPlace` (`-i`) + `backupSuffix`; with `inPlace` or `--debug` the **five sinks refuse**, `run` only | 0→0; 1, 2, 4 → 1 + debug; 127 base; a script's `q N`/`Q N` (any other N) → 1 silent, raw in `lastRc` | **every option before the first `-e`** (sed compiles each `-e` as it reads it), then the `-e` list, `-f`, `--`, paths; `inPlace`/`nullData` derive `-b` (text mode rewrites CRLF on disk); a deny-list keeps typed options out of `addArg`; added the `TUTIL_OUT_SUFFIXES` registry — see [`../../tsed/README.md`](../../tsed/README.md) |
| `tawk` (built) | GNU Awk 5.0.0 (Git for Windows, bash 5.2.37) / 5.4.0 (msys64, bash 5.3.9) | `program` (the first `-e`) **plus** `addProgram`/`clearPrograms` and `programFile` (`-f`); `fieldSep` (`-F`); **`setVar NAME VALUE`** / `clearVars` — **verbatim** (`-v NAME=enc(VALUE)`, after the extras, insertion order); `nullData` (RS/ORS NUL → derived `-0`), `binary` (`-v BINMODE=3`), **`sandbox` (default 1, fail-closed)**, `inPlace` (the inplace include by **absolute** path; requires `sandbox = 0`) + `backupSuffix`; with `inPlace` the **five sinks refuse**, `run` only; `TAwk.apply PROGRAM PATH...` | 0→0; 1, 2 → 1 + debug; 127 base; a program's `exit N` (any other N) → 1 silent, raw in `lastRc` | empty chunks never emitted and no program is rc 2 (gawk would compile the first path); an assignment-looking path (`k=v`, `a::b=v`) is rc 2; the deny-list follows gawk's getopt (any prefix, `-W`, `--`, non-option words, dangling `-v`/`-W`); `inPlace`/`nullData` derive `BINMODE=3`; a missing input file is FATAL; registers `__taw_` and `_progs _vnames _vvals` — see [`../../tawk/README.md`](../../tawk/README.md) |
