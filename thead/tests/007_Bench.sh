#!/bin/bash
# 007_Bench.sh — thead P1: the PLAN.md §5 P1 performance gate, as assertions.
#
# TWO SETS OF NUMBERS, on purpose.
#
#   `../bench.sh`, run BY HAND on an idle machine, measures and prints the real
#   PLAN §5 P1 gate: `THead.take N FILE` <= **1.5x** a bare `head -n N -- FILE`
#   over the same corpus, from the MEDIANS of 21 INTERLEAVED runs. Those are the
#   numbers README.md §8 publishes.
#
...
#   ratio do NOT inflate together under that load: the bare `head` is its own
...
#   of per call, a `$( )` around the producer, a second `head` process per call
...
# exists there (PLAN §8 finding 7: a non-interleaved loop read 2.4x on code that
# interleaved medians put at 1.07-1.30x).
#
# WHY CORPUS SIZE IS NOT THE KNOB. A whole `head` run — one msys process start
# plus the scan — costs 33-36 ms here at 10 000 lines; the wrapper's entire
# fixed delta (`new` + `argv` + `delete`) is 3.1-3.3 ms and is paid once per
# CALL. The ratio is therefore ~(34 + 3.2)/34 — it sits near 1.1x and can only
...
#   A  the take gate — `THead.take` against a bare `head -n` in the two shapes
...
#   D  zero forks, for every member of the surface plus `take`
#
...
TH_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
source "$TH_DIR/thead.sh"
source "$TH_DIR/../tstopwatch/tstopwatch.sh"
...
HEAD_VER="$(head --version 2>/dev/null | sed -n 1p || :)"
if [[ "$HEAD_VER" == "head (GNU coreutils) "* ]]; then
    GNU_OK=1
    kt_test_pass "$HEAD_BIN — $HEAD_VER"
else
    kt_test_pass "SKIP: non-GNU head ($HEAD_BIN — '${HEAD_VER:-no banner}'); every timed case below is skipped"
fi
...
# ===========================================================================
kt_test_section "A. the take gate — THead.take vs a bare \`head -n\`"
# ===========================================================================

# gate_case N EXPECTED_RECORDS — the interleaved-median assertion, 10x ceiling.
gate_case() {
    local __n="$1" __want="$2"
    if ! tcase "THead.take $__n costs at most 10x a bare \`head -n $__n\` (PLAN gate 1.5x)"; then
        return 0
    fi
    ...
    # the gate is about.
    local __nb __nw
    __nb="$( head -n "$__n" -- "$BIG" | wc -l )"; __nb="${__nb//[[:space:]]/}"
    __nw="$( THead.take "$__n" "$BIG" | wc -l )"; __nw="${__nw//[[:space:]]/}"
    ...
    if [[ "$__nb" == "$__want" && "$__nw" == "$__want" ]] && (( __m_take <= __lim )); then
        kt_test_pass "both delivered $__want records; head median ${__m_bare}us, take median ${__m_take}us — ${__r100}% (ceiling ${__lim}us), $NR runs"
    else
        kt_test_fail "head records='$__nb' take records='$__nw' (want $__want); head=${__m_bare}us take=${__m_take}us (${__r100}%) ceiling=${__lim}us"
    fi
}
...
if tcase "the take delta IS one instance: new + argv + delete under 50 ms"; then
    ...
    per=$(( (t1 - t0) / ND ))
    # Idle: 3.1-3.3 ms on both bashes (the plan estimated ~3.9 ms). The ceiling
    ...
    if (( per <= 50000 )); then
        kt_test_pass "new + argv + delete ${per}us/call over $ND calls (ceiling 50000us; 3100-3300us idle)"
    else
...
n_argv="$RESULT"
want="bench_counted -q -n 5 -- $BIG"
if (( BENCH_RAN == 0 )) && [[ "$BASHPID" == "$p0" && "$n_build" == "6" && "$n_argv" == "6" \
      && "${BW[*]}" == "$want" && "${#BW[@]}" == "6" ]]; then
    kt_test_pass "cmd invoked 0 times in $(( ND * 2 )) builds, BASHPID $p0 unchanged, 6 words in the pinned order"
else
...
if [[ "${#BW2[@]}" == "6" && "${BW2[*]}" == "$want" && "$RESULT" == "6" ]]; then
    kt_test_pass "still 6 words after $(( ND * 2 + 1 )) rebuilds"
else
...
# 5000-record figure (14.2x on this box), and what this case is for is catching
# a per-record path that grew an ORDER of magnitude — a fork per record, a
# re-`build` per record — not the known constant.
CNT_N=200

if tcase "h.count over $CNT_N records costs at most 10x \`head -n $CNT_N | wc -l\` (published, not the gate)"; then
    THead.new BQ "$CNT_N" "$BIG"
    declare -a T_WC=() T_CNT=()
    n_wc="$( head -n "$CNT_N" -- "$BIG" | wc -l )"; n_wc="${n_wc//[[:space:]]/}"
    BQ.count >/dev/null 2>&1 || :
    n_cnt=''
    for (( i = 0; i < NR; i++ )); do
        TStopwatch.getTimeStamp; t0=$RESULT
        head -n "$CNT_N" -- "$BIG" | wc -l >/dev/null || :
        TStopwatch.getTimeStamp; t1=$RESULT
        T_WC+=( $(( t1 - t0 )) )
    ...
    if [[ "$n_wc" == "$CNT_N" && "$n_cnt" == "$CNT_N" ]] && (( m_cnt <= lim )); then
        kt_test_pass "both counted $CNT_N; wc median ${m_wc}us, h.count median ${m_cnt}us — ${r100}% (ceiling ${lim}us)"
    else
...
# ===========================================================================
kt_test_section "D. zero forks — every member, \`take\`, the callback and .Add"
# ===========================================================================
...
TForkTH007.Create() { return 0; }
TForkTH007.Add()    { ADDPIDS+=( "$BASHPID" ); return 0; }
build TForkTH007
TForkTH007.new F7

if tcase "the five sinks: the callback and \`.Add\` run in THIS process"; then
    THead.new FH 2 "$FX/two.txt"
    p0=$BASHPID
    CBPIDS=(); ADDPIDS=()
    declare -a FARR=()
    FH.each    pid_cb; rc1=$?
    ...
    if (( ok == 1 && ${#CBPIDS[@]} == 2 && ${#ADDPIDS[@]} == 2 )) \
       && [[ "$BASHPID" == "$p0" && $rc1 -eq 0 && $rc2 -eq 0 && $rc3 -eq 0 && $rc4 -eq 0 && $rc5 -eq 0 \
        ...
        kt_test_pass "2 records per sink, callback and .Add both in pid $p0"
    else
...
if tcase "the ONLY fork per call is head: the builder members and \`take\` fork nothing"; then
    THead.new FB 2 "$FX/two.txt"
    p0=$BASHPID
    declare -a FW=()
    ok=1
    ...
    THead.take 2 "$FX/two.txt" >/dev/null; rct=$?
    [[ "$BASHPID" == "$p0" ]] || ok=0
