...
if TRegEx.isMatch "abc" "b"; then :; fi
TRegEx.match "abc" "b" >/dev/null || :
declare -a m=(); TRegEx.matches "a1b2" "[0-9]" m >/dev/null || :
declare -a p=(); TRegEx.split "a,b,c" "," p >/dev/null || :
TRegEx.replace "a1b2" "[0-9]" "#" >/dev/null
TRegEx.escape "a.b" >/dev/null'

# --- 2. numeric arguments (D1) ---------------------------------------------
kt_test_start "maxCount rejects an injection shape [tregex T2]"
canary="$TMP/pwn"; rm -f "$canary"
bad="x[\$(touch '$canary')]"
declare -a parts=()
TRegEx.split "a,b,c" "," parts "$bad" >/dev/null 2>&1 && accepted="split "
TRegEx.replace "a1b2" "[0-9]" "#" "$bad" >/dev/null 2>&1 && accepted+="replace "
...
out="$(bash -c "set -e
source '$UNIT'
TRegEx.match abc x >/dev/null || printf 'reached rc=%s' \0
printf ' end'" 2>&1)"
