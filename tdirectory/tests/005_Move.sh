#!/bin/bash
# 005_Move.sh - Test TDirectory.Move method
...
test_start "Move - move simple directory"
...
if [[ ! -d "$source_dir" && -d "$dest_dir" && -f "$dest_dir/file.txt" ]]; then
    test_pass "Move - move simple directory"
else
    test_fail "Move - move simple directory (expected source gone, destination exists with content)"
fi

# Test 2: Rename directory (move in same location)
test_start "Move - rename directory"
...
if [[ ! -d "$source_dir" && -d "$dest_dir" && -f "$dest_dir/data.txt" ]]; then
    test_pass "Move - rename directory"
else
    test_fail "Move - rename directory (expected renaming to succeed)"
fi

# Test 3: Move directory with subdirectories
test_start "Move - move directory with subdirectories"
...
if [[ ! -d "$source_dir" && -d "$dest_dir/sub1/sub2" && -f "$dest_dir/sub1/sub2/test.txt" ]]; then
    test_pass "Move - move directory with subdirectories"
else
    test_fail "Move - move directory with subdirectories (expected nested structure to be moved)"
fi

# Test 4: Move directory to nested path
test_start "Move - move directory to nested destination"
...
tdirectory.move "$source_dir" "$dest_dir"
if [[ ! -d "$source_dir" && -d "$dest_dir" && -f "$dest_dir/file.txt" ]]; then
    test_pass "Move - move directory to nested destination"
else
    test_fail "Move - move directory to nested destination (expected move to nested path)"
fi

# Test 5: Move preserves directory structure
test_start "Move - preserves directory structure"
...
if [[ ! -d "$source_dir" && -d "$dest_dir/a/b/c" && -d "$dest_dir/x/y" ]]; then
    test_pass "Move - preserves directory structure"
else
    test_fail "Move - preserves directory structure (expected complete structure preserved)"
fi

# Test 6: Move multiple files integrity
test_start "Move - preserves multiple files"
...
if [[ ! -d "$source_dir" && -f "$dest_dir/file_1.txt" && -f "$dest_dir/file_5.txt" ]]; then
    test_pass "Move - preserves multiple files"
else
    test_fail "Move - preserves multiple files (expected all files to be moved)"
fi

# Test 7: Move directory with special characters
test_start "Move - move directory with special characters"
...
if [[ ! -d "$source_dir" && -d "$dest_dir" && -f "$dest_dir/file-name.txt" ]]; then
    test_pass "Move - move directory with special characters"
else
    test_fail "Move - move directory with special characters (expected move with special chars)"
fi

# Test 8: Move to path with spaces
test_start "Move - move to path with spaces"
...
tdirectory.move "$source_dir" "$dest_dir"
if [[ ! -d "$source_dir" && -d "$dest_dir" && -f "$dest_dir/file.txt" ]]; then
    test_pass "Move - move to path with spaces"
else
    test_fail "Move - move to path with spaces (expected move to path with spaces)"
fi
