...
source_dir="$KK_TEST_TMPDIR/source_move"
dest_dir="$KK_TEST_TMPDIR/dest_move"
...
source_dir="$KK_TEST_TMPDIR/original_name"
dest_dir="$KK_TEST_TMPDIR/renamed_name"
tdirectory.createDirectory "$source_dir"
...
source_dir="$KK_TEST_TMPDIR/source_nested_move"
dest_dir="$KK_TEST_TMPDIR/dest_nested_move"
...
source_dir="$KK_TEST_TMPDIR/source_to_nested"
dest_parent="$KK_TEST_TMPDIR/parent_for_move"
...
source_dir="$KK_TEST_TMPDIR/source_struct_move"
dest_dir="$KK_TEST_TMPDIR/dest_struct_move"
...
source_dir="$KK_TEST_TMPDIR/source_multifile_move"
dest_dir="$KK_TEST_TMPDIR/dest_multifile_move"
tdirectory.createDirectory "$source_dir"
for i in {1..5}; do
...
source_dir="$KK_TEST_TMPDIR/source-special.move"
dest_dir="$KK_TEST_TMPDIR/dest-special.move"
tdirectory.createDirectory "$source_dir"
...
source_dir="$KK_TEST_TMPDIR/source_spaces_move"
dest_dir="$KK_TEST_TMPDIR/destination with spaces"
tdirectory.createDirectory "$source_dir"
echo "content" > "$source_dir/file.txt"
tdirectory.move "$source_dir" "$dest_dir"
if [[ ! -d "$source_dir" && -d "$dest_dir" && -f "$dest_dir/file.txt" ]]; then
