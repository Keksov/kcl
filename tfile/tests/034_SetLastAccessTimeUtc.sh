...
echo "content" > test_setaccess_utc.tmp
now=$(date +%s)
result=$(tfile.setLastAccessTimeUtc "test_setaccess_utc.tmp" "$now")
if [[ $? -eq 0 ]]; then
...
if ! result=$(tfile.setLastAccessTimeUtc "nonexist.tmp" "$now" 2>&1); then
