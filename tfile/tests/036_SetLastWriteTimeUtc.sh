...
echo "content" > test_setwrite_utc.tmp
now=$(date +%s)
result=$(tfile.setLastWriteTimeUtc "test_setwrite_utc.tmp" "$now")
if [[ $? -eq 0 ]]; then
...
if ! result=$(tfile.setLastWriteTimeUtc "nonexist.tmp" "$now" 2>&1); then
