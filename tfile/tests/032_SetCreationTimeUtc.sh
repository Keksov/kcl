...
rc=0
out="$(tfile.setCreationTimeUtc "$F" "$EPOCH" 2>&1)" || rc=$?
...
rc=0
tfile.setCreationTimeUtc "$_KT_TMPDIR/nonexist.tmp" "$EPOCH" >/dev/null 2>&1 || rc=$?
...
RESULT="__unset__"
tfile.setCreationTimeUtc "$F" "$EPOCH" >/dev/null 2>&1 || :
if [[ -z "$RESULT" ]]; then
