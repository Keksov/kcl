...
kt_test_start "Get creation time of non-existing file"
if ! result=$(tfile.getCreationTime "$_KT_TMPDIR/nonexist.tmp" 2>&1); then
    kt_test_pass "Get creation time of non-existing file (correctly failed)"
else
    kt_test_fail "Get creation time of non-existing file (should have failed)"
fi
