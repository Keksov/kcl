...
printf "initial" > "$TEST_TMP_DIR/append_text.tmp"
writer=$(tfile.appendText "$TEST_TMP_DIR/append_text.tmp")
...
# close_writer "$writer"
if [[ "$(cat "$TEST_TMP_DIR"/append_text.tmp)" == "initial appended" ]]; then
    test_pass "Append text to existing file"
else
    test_fail "Append text to existing file (got: $(cat "$TEST_TMP_DIR"/append_text.tmp))"
fi

# Test 2: Append to new file
test_start "Append text to new file"
rm -f "$TEST_TMP_DIR/new_append.tmp"
writer=$(tfile.appendText "$TEST_TMP_DIR/new_append.tmp")
printf "new content" > "$writer"
# close_writer "$writer"
if [[ "$(cat "$TEST_TMP_DIR"/new_append.tmp)" == "new content" ]]; then
