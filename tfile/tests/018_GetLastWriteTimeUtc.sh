...
kt_test_start "Get last write time UTC of non-existing file"
if ! result=$(tfile.getLastWriteTimeUtc "$_KT_TMPDIR/nonexist.tmp" 2>&1); then
    kt_test_pass "Get last write time UTC of non-existing file (correctly failed)"
else
    kt_test_fail "Get last write time UTC of non-existing file (should have failed)"
fi
