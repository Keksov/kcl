...
math.min 1 2 >/dev/null
math.max 1.5 2.5 >/dev/null
math.sumInt 1 2 3 >/dev/null
math.divMod 7 2 >/dev/null
math.sign -3 >/dev/null'

# --- 2. numeric arguments (D1) ---------------------------------------------
kt_test_start "an injection-shaped number is rejected and never evaluated [M4]"
canary="$TMP/pwn"; rm -f "$canary"
bad="x[\$(touch '$canary')]"
accepted=""
math.divMod "$bad" 1 >/dev/null 2>&1 && accepted+="divMod "
math.sumInt "$bad" >/dev/null 2>&1 && accepted+="sumInt "
math.minIntValue "$bad" 1 >/dev/null 2>&1 && accepted+="minIntValue "
math.maxIntValue "$bad" 1 >/dev/null 2>&1 && accepted+="maxIntValue "
math.randomRange "$bad" 5 >/dev/null 2>&1 && accepted+="randomRange "
...
kt_test_start "leading-zero operands are decimal, not octal [M4]"
r1="$(math.min 08 5)"; r2="$(math.sumInt 08 09)"; r3="$(math.minIntValue 08 5)"
if [[ "$r1" == "5" && "$r2" == "17" && "$r3" == "5" ]]; then
    kt_test_pass "min=5 sumInt=17 minIntValue=5"
else
    kt_test_fail "min='$r1' sumInt='$r2' minIntValue='$r3'"
fi

# --- 3. Tier-A helpers must not abort the caller under set -e (M7) ---------
expect_clean "comparison helpers survive set -e [M7]" '
set -e
math.min 1.5 2 >/dev/null
math.max 1.5 2 >/dev/null
math.sign 0 >/dev/null
math.compareValue 1 1 >/dev/null'
...
out="$(bash -c "set -e
source '$UNIT'
math.divMod 1 0 >/dev/null || printf 'reached rc=%s' \0
printf ' end'" 2>&1)"
