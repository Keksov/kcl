...
| [thashset](thashset/README.md) | `THashSet` | instance | §1 in full **for what is implemented**. The unit is at **P2 of its own roadmap** (`thashset_ledger.json`): the membership core, `ToArray`, `ForEach`, `Assign`, both `AddRange` forms and the four set-algebra operations are real; P3 (events: `Notify`/`on_notify`) and P4 (bench/docs closeout) are not written yet. No `TObjectHashSet` exists in FPC |
