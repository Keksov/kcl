...
| [thashset](thashset/README.md) | `THashSet` | instance | §1 in full **for what is implemented**. The unit is at **P3 of its own roadmap** (`thashset_ledger.json`): every declared member has a real body — the membership core, `ToArray`/`ForEach`/`Assign`, both `AddRange` forms, the four set-algebra operations and the event seam (`on_notify`/`onNotify`/`Notify`/`_notifyHook`); only P4 (bench/docs closeout) is not done yet. No `TObjectHashSet` exists in FPC |
