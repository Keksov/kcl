...
kk_test_section "003: Insert Operations"

# Create TList instance
TList.new testlist

# Test: Insert at beginning
kk_test_start "Insert at beginning"
testlist.Insert 0 "first"
count=$(testlist.count)
declare -n items_ref="testlist_items"
if [[ "$count" == "1" && "${items_ref[0]}" == "first" ]]; then
    kk_test_pass "Insert at beginning successful"
else
    kk_test_fail "Insert failed: count=$count, item=${items_ref[0]}"
fi
...
if [[ "$count" == "2" && "${items_ref[1]}" == "last" ]]; then
    kk_test_pass "Insert at end successful"
else
    kk_test_fail "Insert failed: count=$count, item=${items_ref[1]}"
fi
...
if [[ "$count" == "3" && "${items_ref[0]}" == "first" && "${items_ref[1]}" == "middle" && "${items_ref[2]}" == "last" ]]; then
    kk_test_pass "Insert in middle successful"
else
    kk_test_fail "Insert failed: count=$count, items: [0]=${items_ref[0]}, [1]=${items_ref[1]}, [2]=${items_ref[2]}"
fi

# Test: Insert with capacity growth
kk_test_start "Insert with capacity growth"
...
if [[ "$count" == "10" && "$capacity" -ge "10" ]]; then
    kk_test_pass "Insert with growth successful: count=$count, capacity=$capacity"
else
    kk_test_fail "Growth failed: count=$count, capacity=$capacity"
fi
...
kk_test_start "Invalid index - too large"
result=$(testlist.Insert 20 "invalid" 2>&1)
exit_code=$?
if [[ $exit_code -ne 0 ]]; then
    kk_test_pass "Correctly rejected too large index"
else
    kk_test_fail "Should have rejected too large index"
fi

# Cleanup
testlist.delete

kk_test_log "003_Insert_Operations.sh completed"
