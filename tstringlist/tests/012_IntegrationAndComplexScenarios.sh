...
kt_test_section "012: Integration and Complex Scenarios"

# Test: Build sorted list from unsorted data
kt_test_start "Build sorted list from unsorted data"
...
if [[ "$(mylist.sorted)" == "true" && "$(mylist.count)" == "5" ]]; then
    kt_test_pass "Unsorted list sorted and still has all items"
else
    kt_test_fail "Sort failed: sorted=$(mylist.sorted), count=$(mylist.count)"
fi

# Test: Find-Add workflow
kt_test_start "Find item, then add related items"
...
if [[ "$index" == "1" ]]; then
    list.Add "apricot"
    list.Sort
    kt_test_pass "Found and sorted list successfully"
else
    kt_test_fail "Could not find banana"
fi

# Test: Copy and modify
kt_test_start "Copy list and modify independently"
...
if [[ "$orig_item" == "first" && "$copy_item" == "changed" ]]; then
    kt_test_pass "Copy and modify independent"
else
    kt_test_fail "orig=$orig_item, copy=$copy_item"
fi
original.delete
copy.delete

# Test: Remove while iterating (simulated)
kt_test_start "Remove multiple items"
TStringList.new mylist
mylist.Add "keep1"
mylist.Add "remove1"
mylist.Add "keep2"
...
if [[ "$count" == "3" && "$items_correct" == "true" ]]; then
    kt_test_pass "Multiple removes maintain correct items"
else
    kt_test_fail "Count=$count, items_correct=$items_correct"
fi
mylist.delete

# Test: Case-sensitive to case-insensitive transition
kt_test_start "Switch case sensitivity"
TStringList.new mylist
mylist.Add "ABC"
...
if [[ "$index1" == "-1" && "$index2" == "0" ]]; then
    kt_test_pass "Case sensitivity switch affects search correctly"
else
    kt_test_fail "case_sensitive=$index1, case_insensitive=$index2"
fi
mylist.delete
...
if [[ "$count" == "100" && "$first" == "item_1" && "$last" == "item_100" ]]; then
    kt_test_pass "Large list (100 items) operations work correctly"
else
...
kt_test_start "Stress test - rapid operations"
TStringList.new mylist
for i in {1..50}; do
    mylist.Add "item$i"
done
...
count=$(mylist.count)
if [[ "$count" == "25" ]]; then
    kt_test_pass "Stress test passed (add 50, remove 25)"
else
    kt_test_fail "count=$count, expected 25"
fi
mylist.delete

# Test: Clear and reuse
kt_test_start "Clear and reuse list multiple times"
TStringList.new mylist
for cycle in {1..3}; do
...
if [[ "$count" == "2" ]]; then
    kt_test_pass "Clear and reuse works multiple times"
else
    kt_test_fail "count=$count, expected 2"
fi
mylist.delete

# Test: Complex property combination
kt_test_start "Complex property combination workflow"
TStringList.new mylist
mylist.case_sensitive = "false"
mylist.sorted = "true"
mylist.duplicates = "dupIgnore"
...
if [[ "$count" == "4" && "$first" == "apple" ]]; then
    kt_test_pass "Complex property combination works"
else
    kt_test_fail "count=$count (expected 4), first=$first (expected apple)"
fi
mylist.delete

kt_test_log "012_IntegrationAndComplexScenarios.sh completed"
