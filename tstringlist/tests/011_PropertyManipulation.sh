...
kt_test_section "011: Property Manipulation"

# Test: Get case_sensitive property
kt_test_start "Get case_sensitive property"
TStringList.new mylist
prop=$(mylist.case_sensitive)
if [[ "$prop" == "false" ]]; then
    kt_test_pass "case_sensitive is 'false' by default"
else
    kt_test_fail "case_sensitive is '$prop', expected 'false'"
fi

# Test: Set case_sensitive to true
kt_test_start "Set case_sensitive to true"
mylist.case_sensitive = "true"
prop=$(mylist.case_sensitive)
if [[ "$prop" == "true" ]]; then
    kt_test_pass "case_sensitive set to 'true'"
else
    kt_test_fail "case_sensitive is '$prop', expected 'true'"
fi

# Test: Set case_sensitive to false
kt_test_start "Set case_sensitive to false"
mylist.case_sensitive = "false"
prop=$(mylist.case_sensitive)
if [[ "$prop" == "false" ]]; then
    kt_test_pass "case_sensitive set to 'false'"
else
    kt_test_fail "case_sensitive is '$prop', expected 'false'"
fi

# Test: Get sorted property
kt_test_start "Get sorted property"
prop=$(mylist.sorted)
if [[ "$prop" == "false" ]]; then
    kt_test_pass "sorted is 'false' by default"
else
    kt_test_fail "sorted is '$prop', expected 'false'"
fi

# Test: Set sorted to true
kt_test_start "Set sorted to true"
mylist.sorted = "true"
prop=$(mylist.sorted)
if [[ "$prop" == "true" ]]; then
    kt_test_pass "sorted set to 'true'"
else
    kt_test_fail "sorted is '$prop', expected 'true'"
fi

# Test: Set sorted to false
kt_test_start "Set sorted to false"
mylist.sorted = "false"
prop=$(mylist.sorted)
if [[ "$prop" == "false" ]]; then
    kt_test_pass "sorted set to 'false'"
else
    kt_test_fail "sorted is '$prop', expected 'false'"
fi

# Test: Get duplicates property
kt_test_start "Get duplicates property"
prop=$(mylist.duplicates)
if [[ "$prop" == "dupAccept" ]]; then
    kt_test_pass "duplicates is 'dupAccept' by default"
else
    kt_test_fail "duplicates is '$prop', expected 'dupAccept'"
fi

# Test: Set duplicates to dupIgnore
kt_test_start "Set duplicates to dupIgnore"
mylist.duplicates = "dupIgnore"
prop=$(mylist.duplicates)
if [[ "$prop" == "dupIgnore" ]]; then
    kt_test_pass "duplicates set to 'dupIgnore'"
else
    kt_test_fail "duplicates is '$prop', expected 'dupIgnore'"
fi

# Test: Set duplicates to dupError
kt_test_start "Set duplicates to dupError"
mylist.duplicates = "dupError"
prop=$(mylist.duplicates)
if [[ "$prop" == "dupError" ]]; then
    kt_test_pass "duplicates set to 'dupError'"
else
    kt_test_fail "duplicates is '$prop', expected 'dupError'"
fi

# Test: Set duplicates to dupAccept
kt_test_start "Set duplicates to dupAccept"
mylist.duplicates = "dupAccept"
prop=$(mylist.duplicates)
if [[ "$prop" == "dupAccept" ]]; then
    kt_test_pass "duplicates set to 'dupAccept'"
else
    kt_test_fail "duplicates is '$prop', expected 'dupAccept'"
fi

# Test: Get count property
kt_test_start "Get count property"
mylist.Add "item1"
mylist.Add "item2"
count=$(mylist.count)
if [[ "$count" == "2" ]]; then
    kt_test_pass "count property returns correct value"
else
    kt_test_fail "count is '$count', expected '2'"
fi

# Test: Get capacity property
kt_test_start "Get capacity property"
capacity=$(mylist.capacity)
if [[ "$capacity" -ge "2" ]]; then
    kt_test_pass "capacity property returns valid value"
else
    kt_test_fail "capacity is '$capacity', expected >= 2"
fi

# Test: Properties affect behavior - case sensitivity
kt_test_start "case_sensitive property affects IndexOf"
...
if [[ "$index1" == "0" && "$index2" == "-1" ]]; then
    kt_test_pass "case_sensitive property affects IndexOf behavior"
else
    kt_test_fail "case_insensitive=$index1, case_sensitive=$index2"
fi
caselist.delete

# Test: Properties affect behavior - sorted/duplicates
kt_test_start "sorted and duplicates properties affect Add"
...
if [[ "$count" == "2" ]]; then
    kt_test_pass "duplicates and sorted properties work correctly"
else
    kt_test_fail "count is '$count', expected '2'"
fi
proplist.delete
...
kt_test_start "Multiple rapid property changes"
...
if [[ "$cs" == "false" && "$s" == "false" && "$d" == "dupAccept" ]]; then
    kt_test_pass "Multiple property changes work correctly"
else
    kt_test_fail "Final state: cs=$cs, s=$s, d=$d"
fi
multilist.delete

# Test: Properties are instance-specific
kt_test_start "Properties are instance-specific"
...
if [[ "$cs1" == "true" && "$cs2" == "false" && "$s1" == "true" && "$s2" == "false" ]]; then
    kt_test_pass "Properties are independent per instance"
else
    kt_test_fail "Properties not independent: list1.cs=$cs1, list2.cs=$cs2"
fi
list1.delete
list2.delete
...
kt_test_log "011_PropertyManipulation.sh completed"
