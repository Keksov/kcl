...
if [[ -z "$SCRIPT_DIR" || "$SCRIPT_DIR" == *"/tstringlist" ]]; then
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
fi
KCL_DIR="$SCRIPT_DIR/.."
TSTRINGLIST_SCRIPT="$KCL_DIR/tstringlist.sh"
...
# Source the tstringlist script if not already sourced
if ! declare -F | grep -q "tstringlist.copy"; then
    source "$TSTRINGLIST_SCRIPT"
fi
