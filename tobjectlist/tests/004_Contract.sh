...
bad="x[\$(touch '$canary')]"
TObjectList.new L; L.Add a; L.Add b
accepted=""
L.Delete "$bad" 2>/dev/null && accepted+="Delete "
L.Put "$bad" v 2>/dev/null && accepted+="Put "
L.BatchDelete "$bad" 1 2>/dev/null && accepted+="BatchDelete "
L.FindInstanceOf TList true "$bad" 2>/dev/null && accepted+="FindInstanceOf "
...
out="$(bash -c "set -e
source '$UNIT'
TObjectList.new L; L.Delete 5 || printf 'reached rc=%s' \0
printf ' end'" 2>&1)"
